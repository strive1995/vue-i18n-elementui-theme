...
(function($){
	...
	view = {
		...
		appendNodes: function(setting, level, nodes, parentNode, index, initFlag, openFlag) {
			...
			var maxNameSize = 0, asyncSetting = setting.async;
			if(asyncSetting != null && asyncSetting != undefined){
				maxNameSize = asyncSetting.otherParam.maxNameSize;
				maxNameSize = parseInt(maxNameSize);
			}
			...
			for (var i = 0, l = nodes.length; i < l; i++) {
				...
				if (openFlag) {
					...
					view.makeDOMNodeIcon(html, setting, node, maxNameSize);
		...
		makeDOMNodeIcon: function(html, setting, node, maxNameSize) {
			var nameStr = data.getNodeName(setting, node),
			name = setting.view.nameIsHTML ? nameStr : nameStr.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');

			//截取字符数
			if(maxNameSize != 0 && (name.length > maxNameSize)){
